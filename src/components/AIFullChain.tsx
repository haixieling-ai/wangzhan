...
const PROJECT_06_IMAGES: ConceptImage[] = Array.from({ length: 40 }, (_, i) => ({
  src: `/projects/project-06/settings/${i + 1}.jpg`,
  prompt: `项目二设定图 ${i + 1}`,
}));
...
export function AIFullChain({ conceptImages, videoSrc, secondVideoSrc, secondConceptImages, thirdVideoSrc, thirdConceptImages, fourthVideoSrc, fourthConceptImages, fifthVideoSrc, fifthConceptImages }: AIFullChainProps) {
  ...
  useEffect(() => {
    ...
    if (v && effectiveSecondVideoSrc) v.play().catch(() => {});
  }, [effectiveSecondVideoSrc]);
  ...
  return (
    ...
    >
      ...
      {/* 1. 视频层 - 第一个视频 */}
      <div data-cursor-invert className="w-full bg-transparent">
        <div className="overflow-hidden bg-transparent">
          <div className="aspect-video w-full bg-transparent">
            <video
              ...
              onError={() => {
                if (effectiveVideoSrc !== FALLBACK_VIDEO) {
                  setEffectiveVideoSrc(FALLBACK_VIDEO);
                }
              }}
            >
            </video>
